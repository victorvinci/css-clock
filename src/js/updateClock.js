const updateClock = () => {
  ...
  function getDateBR() {
    const horarioBR = moment()
      .subtract(1, 'hour')
      .format('HH:mm:ss');
    const horaBR = moment()
      .subtract(1, 'hour')
      .format('HH'); //this exists for conditionals
    const dormirBR = moment()
      .subtract(1, 'hour')
      .to(moment('01:00:00', 'HH:mm:ss').add(1, 'day')); // calculate the amount of hours until the next time to sleep
    ...
    if (horaBR >= 1 && horaBR < 9) {
      ...
      dormeBR.innerHTML =
        ...
        moment()
          .subtract(1, 'hour')
          .to(moment('01:00:00', 'HH:mm:ss')); //there is a bug when the moment's clock reachs 19:00. This seems to fix it
      talkBR = true;
    } else {
  ...
  function getDateIT() {
    const horarioIT = moment()
      .add(5, 'hour')
      .format('HH:mm:ss'); // add +5 for Rome timezone
    const horaIT = moment()
      .add(5, 'hour')
      .format('HH'); //this exists for conditionals
    const dormirIT = moment()
      .add(5, 'hour')
      .to(moment('03:00:00', 'HH:mm:ss').add(1, 'day')); // calculate the amount of hours until the next time to sleep
    ...
    if (horaIT >= 3 && horaIT < 11) {
      ...
      dormeIT.innerHTML =
        ...
        moment()
          .add(5, 'hour')
          .to(moment('03:00:00', 'HH:mm:ss')); //prevents odd bug when clock reachs 00:30 IT time
      talkIT = true;
    } else {
  ...
  if (getDateBR() === true && getDateIT() === true) {
    talkTime.innerHTML = 'GOOD TIME TO TALK';
